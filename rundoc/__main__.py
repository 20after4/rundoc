...
_run_control_options = [
    ...
    click.option('-y', '--yes', is_flag=True,
        help="""[Deprecated: this is now default behaviour. See '-a' to disable] Confirm all steps without prompting user.""",
    ),
    click.option('-a', '--ask', count=True,
        help="""Let rundoc ask you for input on variables and/or code. No option: ask for missing vars. -a: ask for all vars. -aa: ask for all vars and modify code on failure. -aaa: ask for all vars and code.""",
    ),
    click.option('-b', '--breakpoint', type=int, multiple=True,
        help="""Step number on which to force prompt for code input. You can use this option multiple times to add multiple breakpoints.""",
    ),
]
...
_tag_options = [
    click.option('-t', '--tags', type=str, default='',
        help="""Hash (#) separated list of tags, e.g. '-t bash#proj-2#test_3'. Filter out all code blocks that don't contain at least one of the listed tags. (Works like OR logic).""",
    ),
    click.option('-T', '--must-have-tags', type=str, default='',
        help="""Hash (#) separated list of tags, e.g. '-t bash#proj-2#test_3)'. Filter out all code blocks that are missing at least one of the listed tags. (Works like AND logic).""",
    ),
    click.option('-N', '--must-not-have-tags', type=str, default='',
        help="""Hash (#) separated list of tags, e.g. '-t bash#proj-2#test_3)'. Filter out all code blocks that contain at least one of the listed tags. (Works like AND NOT logic).""",
    ),
]
